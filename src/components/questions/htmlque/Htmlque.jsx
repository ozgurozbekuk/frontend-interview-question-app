...
const homeIcon = <FontAwesomeIcon icon={faHouse} />
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('./data/questions/html.json');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const result = await response.json();
        setHtmlQuestion(result);
  ...
  const firstQuestion =htmlQuestion.length > 0 ? htmlQuestion[quesNumber] : "";
