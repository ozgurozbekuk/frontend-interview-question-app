...
function Nodejsque( ) {
  ...
  const [nodejsQuestion,setNodejsQuestion] = useState([]);
...
const homeIcon = <FontAwesomeIcon icon={faHouse} />
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('./data/questions/nodejs.json');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const result = await response.json();
        setNodejsQuestion(result);
  ...
  const firstQuestion =nodejsQuestion.length > 0 ? nodejsQuestion[quesNumber] : "";
