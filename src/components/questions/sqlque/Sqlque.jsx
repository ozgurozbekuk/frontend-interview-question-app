...
const homeIcon = <FontAwesomeIcon icon={faHouse} />
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('./data/questions/sql.json');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const result = await response.json();
        setSqlQuestion(result);
  ...
  const firstQuestion =sqlQuestion.length > 0 ? sqlQuestion[quesNumber] : "";
