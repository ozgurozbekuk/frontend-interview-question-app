...
const homeIcon = <FontAwesomeIcon icon={faHouse} />
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch('./data/questions/js.json');
        if (!response.ok) {
          throw new Error('Network response was not ok');
        }
        const result = await response.json();
        setJsQuestion(result);
  ...
  const firstQuestion =jsQuestion.length > 0 ? jsQuestion[quesNumber] : "";
  ...
  const handleKeyDown = (event) => {
    if (event.key === 'ArrowRight') {
      ...
      setQuesNumber((prevQuesNumber) => {
        return prevQuesNumber < 199 ? prevQuesNumber + 1 : 0;
      });
    } else if (event.key === 'ArrowLeft') {
      setIsVisible(false)
      setQuesNumber((prevQuesNumber) => {
        return prevQuesNumber > 0 ? prevQuesNumber - 1 : 199;
      });
    }
  };
 
  function clickHandleRight() {
    if(firstQuestion.id < 200) 
